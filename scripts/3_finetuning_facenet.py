...
class FaceDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        ...
        # Tự động lấy danh sách nhãn từ các thư mục con trong dataset/
        self.labels = [d for d in os.listdir(data_dir) if os.path.isdir(os.path.join(data_dir, d))]
        if not self.labels:
            raise ValueError("Không tìm thấy thư mục nhãn nào trong dataset/. Vui lòng kiểm tra lại dữ liệu.")
        
        self.label_to_idx = {label: idx for idx, label in enumerate(self.labels)}
        self.image_paths = []
        self.image_labels = []

        # Load tất cả ảnh từ dataset
        ...
        if not self.image_paths:
            raise ValueError("Không tìm thấy ảnh nào trong dataset/. Vui lòng kiểm tra lại dữ liệu.")
...
# Khởi tạo mô hình với số lớp bằng số nhãn tìm được
